...
const Swiper = dynamic(() => import('swiper/react').then(mod => ({ default: mod.Swiper })), {
  ssr: false,
  loading: () => <div className="h-screen bg-gray-900 flex items-center justify-center">
...
interface HeroImageSliderDynamicProps {
  className?: string;
}

export default function HeroImageSliderDynamic({ className = '' }: HeroImageSliderDynamicProps) {
  ...
  useEffect(() => {
    ...
    // Use placeholder images for now to avoid API issues
    const placeholderImages = heroSlidesContent.map((slide, index) => ({
      ...
      alt_description: slide.imageAlt,
  ...
  // Combine slides content with images
  const slides = heroSlidesContent.map((slideContent, index) => {
    const image = images[index] || images[0];
    return {
      ...slideContent,
      image: image ? image.urls.regular : '/images/hero.jpg',
      unsplashImage: image,
    };
  });
